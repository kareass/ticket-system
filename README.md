...
| `npm run api:smoke` | 后端接口冒烟（91 项断言，需服务已在 3000 运行；运行后自动清理测试数据） |
...
2. **需求ID 由用户填写**（全局唯一、必填）；一条工单最多转出一条需求（一对一）。两条等价路径：① 列表操作列「转需求」→ 弹框内填写需求ID（工单已登记编号则回填）；② 编辑工单开启「是否转需求」→ 该开关打开后「需求ID / 需求内容」才可填 → 保存即按编号自动同步创建需求（同步日期/标题/内容/系统，节点默认「方案中」）。已转需求的工单不可重复转换、不可直接删除（需先删除关联需求）；删除需求会复位工单标记并保留其需求ID，便于按原编号重转。
