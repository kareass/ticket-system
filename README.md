...
部门内部**工单统计与需求追踪**系统：登记日常工作工单，一键转为需求，跟进「方案中 → 开发中 → 测试中 → 测试完毕 → 已合并 → 已发布」全流程。
...
| `npm run api:smoke` | 后端接口冒烟（107 项断言，需服务已在 3000 运行；运行后自动清理测试数据） |
...
**无外网的内网 / 隔离网**：用 **[docs/内网部署方案.md](docs/内网部署方案.md)** —— 在能上网的准备机打包，
目标机只需装 Node.js 后 `npm run start`，全程不依赖任何公网服务（含 Prisma 查询引擎的离线处理）。
...
| [内网部署方案.md](docs/内网部署方案.md) | **无外网内网 / 隔离网部署**：离线整包投递、内网源直装、内网 Git 更新、Prisma 引擎离线处理 |
...
2. **需求ID 由用户填写**（全局唯一；手工新建/编辑需求时选填，**工单转需求时必填**）；一条工单最多转出一条需求（一对一）。两条等价路径：① 列表操作列「转需求」→ 弹框内填写需求ID 与需求内容（工单已登记的值则回填）；② 编辑工单开启「是否转需求」→ 该开关打开后「需求ID / 需求内容」才可填 → 保存即按编号自动同步创建需求（同步日期/标题/内容/系统，节点默认「方案中」）。**需求内容不做兜底**：填什么是什么，留空则转出的需求内容也为空（不再回退成工单标题）。已转需求的工单不可重复转换、不可直接删除（需先删除关联需求）；删除需求会复位工单标记并保留其需求ID，便于按原编号重转。

开发时长 `developmentDays` 由服务端自动计算，客户端提交值会被忽略：未发版为「日期 → 今天」，已发版为「日期 → 发版时间」，含当天，下限 0。

## 扩展：新增下拉选项

**所有下拉选项的唯一来源是 `src/types/index.ts`** —— 表单下拉、列表内联下拉、筛选框，以及后端接口的
枚举校验（`src/lib/server/helpers.ts` 转发为 `SYSTEMS` / `WORK_ORDER_STATUSES` / `NODES`）全部取自这里，
改一处即四处同步。数组用 `as const`，联合类型由数组自动推导；配色表为 `Record<联合类型, string>`，
漏加配色直接编译报错。

**不需要数据库迁移**：`schema.prisma` 中 `system` / `status` / `currentNode` 是普通 `String` 列，不是数据库枚举。

| 下拉框 | 数组 |
|--------|------|
| 系统（工单 / 需求共用） | `SYSTEM_OPTIONS` |
| 工单状态 | `WORK_ORDER_STATUS_OPTIONS` |
| 需求当前节点 | `NODE_OPTIONS` ← 取值由设计文档定义，增删需先改文档 |
| 是 / 否（加急、发版） | `YES_NO_OPTIONS` |

改完 `npm run build` + `npm run start` 即可生效（面向管理员的操作步骤见[用户手册 7.1](docs/用户手册.md)）。
